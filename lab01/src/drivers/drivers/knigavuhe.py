...
class KnigaVUhe(BaseDriver[AudioBook]):
    ...
    def get_book(self, url: str):
        ...
        title = book["name"]
        author = find_in_soup(
            soup, "span.book_title_elem > span > a", "unknown_author"
        )
            ...
            if book := self._parse_book_card(card, author, series_name):
                ...
                for element in card.select(
                    ...
                    if url == "":
                        ...
                        book.urls.append(url)
                        book.narrators.append(safe_name(narrator))
                        url = narrator = ""
    ...
    def search_books(self, query, limit=10, offset=0):
        ...
        while True:
            ...
            for card in elements:
                if book := self._parse_book_card(card, "unknown_author", ""):
                    books.append(book)
                if len(books) == limit:
                    break
